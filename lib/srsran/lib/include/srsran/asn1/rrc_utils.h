...
/* PMCH-SoftBufferSizeParameters-r19's scalingFactorBeta enum -> num/den fraction.
 * Decode-only direction (this repo is receive-only, no MCCH encode side). ordinal
 * is the enum's plain options value - kept out of this header's signature since a
 * nested-type reference needs rrc.h's full struct definition, which this header
 * avoids pulling in (mirrors rt-mbms-tx's rrc_utils.h). */
