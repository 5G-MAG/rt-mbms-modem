...
inline uint32_t enum_to_number(const mbsfn_area_info_t::mcch_cfg_t::mod_period_t& mod_period)
{
  /* r16 adds rf256 between rf128 and rf512; r9 only had rf512/rf1024. Spare values → 0. */
  constexpr static uint32_t options[] = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 0, 0, 0, 0, 0};
  return enum_to_number(options, (uint32_t)mbsfn_area_info_t::mcch_cfg_t::mod_period_t::nulltype, (uint32_t)mod_period);
}
