...
typedef struct SRSRAN_API {
  ...
  uint32_t frame_idx;
  uint32_t cas_frame_idx_hint; /* which frame_idx in the window holds the CAS frame (sfn%4==0); updated after each decode; 1 = conservative fallback before first decode */

  cf_t*    cas_syms; /* CAS repetition symbol buffer: 5*PBCH_CAS_NOF_SC (NCP) or 3*PBCH_CAS_NOF_SC (ECP) */
