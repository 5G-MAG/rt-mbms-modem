                                          ...
                                          int                    Ls)
{
  ...
  uint32_t  i, k, L, m;
  uint32_t  Yk, ncce;
  const int nof_candidates[5] = {6, 6, 2, 2, 1}; /* AL16 for MBMS-dedicated PDCCH Format 4 */
  ...
  k = 0;
  // All aggregation levels from 1 to 16 (l=4 = AL16 for MBMS dedicated PDCCH)
  for (l = 0; l <= 4; l++) {
    L = (1 << l);
    if (Ls < 0 || Ls == L) {
...
uint32_t srsran_pdcch_common_locations_ncce(uint32_t nof_cce, srsran_dci_location_t* c, uint32_t max_candidates)
{
  ...
  k = 0;
  for (l = 2; l <= 4; l++) {  /* l=4 = AL16 for MBMS common PDCCH (TS 36.211 §6.7) */
    L = (1 << l);
