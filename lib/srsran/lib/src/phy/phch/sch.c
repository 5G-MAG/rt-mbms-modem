                  ...
                  uint32_t                n_cb_cap)
{
  ...
  for (int cb_idx = 0; cb_idx < cb_segm->C; cb_idx++) {
    ...
    if (softbuffer->cb_crc[cb_idx] == false) {
      ...
      if (e_min > 0) {
        if (srsran_rm_turbo_rx_mch(
                &e_bits_s[rp], softbuffer->buffer_f[cb_idx], n_e2, cb_len_idx, rv, e_min, n_cb_cap)) {
          ERROR("Error in MCH rate matching");
          return SRSRAN_ERROR;
        }
                        ...
                        srsran_uci_value_t* uci_data)
{
  ...
  if (cb_segm.tbs > 0) {
    ...
    ret        = decode_tb(q, cfg->softbuffers.rx, &cb_segm, Qm, cfg->grant.tb.rv, G * Qm, &g_bits[e_offset], data, 0, 0);
  }
  return ret;
}
