...
{
  srsran_scs_t scs    = q->dl_sf.subcarrier_spacing;
   ...
   * Mirrors the formula used in chest_dl estimate_port_mbsfn and the TX enb_dl. */
  uint32_t sf_idx;
...
static void put_mib(srsran_enb_dl_t* q)
{
  ...
  if (sf_idx == 0) {
    ...
    srsran_pbch_encode(&q->pbch, bch_payload, q->sf_symbols, sfn % 4, sfn);
  }
}
