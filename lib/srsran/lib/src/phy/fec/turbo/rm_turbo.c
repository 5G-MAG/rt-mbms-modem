...
static void
srsran_rm_turbo_gentable_mch(uint16_t* table, uint32_t cb_idx, uint32_t rv_idx, uint32_t e_min, uint32_t n_cb_cap)
{
  ...
  /* TS 36.212 §5.1.4.1.2: N_cb = min(floor(N_IR/C), K_w). n_cb_cap (0 = not
   * available/configured) is the caller-precomputed floor(N_IR/C); only apply it
   * when it actually shrinks the buffer below the uncapped K_w. */
  int N_cb = 3 * nrows * NCOLS;
