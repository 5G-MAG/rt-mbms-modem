...
void rlc_um_lte::rlc_um_lte_rx::handle_data_pdu(uint8_t* payload, uint32_t nof_bytes)
{
  ...
  if (getenv("PMCH_TI_DIAG") && cfg.um.is_mrb) {
    fprintf(stderr,
            "TI_DIAG_RLCUM rb=%s sn=%u fi=%u nof_bytes=%u vr_ur=%u vr_uh=%u rx_window_size=%u\n",
            rb_name.c_str(), header.sn, header.fi, nof_bytes, vr_ur, vr_uh, cfg.um.rx_window_size);
  }

  if (RX_MOD_BASE(header.sn) >= RX_MOD_BASE(vr_uh - cfg.um.rx_window_size) &&
      RX_MOD_BASE(header.sn) < RX_MOD_BASE(vr_ur)) {
    if (getenv("PMCH_TI_DIAG") && cfg.um.is_mrb) {
      fprintf(stderr, "TI_DIAG_RLCUM  -> DISCARD outside window\n");
    }
  ...
  if (rx_window.end() != it) {
    if (getenv("PMCH_TI_DIAG") && cfg.um.is_mrb) {
      fprintf(stderr, "TI_DIAG_RLCUM  -> DISCARD duplicate\n");
    }
